...
const HeroNav = ({ isCommercial = false }: HeroNavProps) => {
  ...
  return (
    <div className="absolute top-[60px] sm:top-[80px] md:top-[100px] left-1/2 -translate-x-1/2 w-full flex justify-center z-20 px-4">
      <div
        className={cn(
          'inline-flex flex-col gap-0 pl-3 sm:pl-6 md:pl-[30px] pr-1.5 sm:pr-4 md:pr-[9px] bg-white/20 backdrop-blur-[30px] rounded-full sm:rounded-[24px] md:rounded-[30px] border border-white/22 transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] overflow-hidden py-1.5 sm:py-2 max-w-[calc(100vw-2rem)]'
        )}
        onMouseLeave={() => setHoveredItem(null)}
      >
        <div className="flex flex-row items-center gap-3 sm:gap-4 md:gap-[30px]">
        ...
        <div
          className={cn(
            ...
            hasDropdown
              ? 'max-h-[800px] opacity-100 pt-4 mt-2 border-t border-white/10'
                ...
                return (
                  <div key={link.href}>
                    <Link
                      href={link.href}
                      className={cn(
                        'px-4 py-2 text-white text-sm font-semibold rounded-lg hover:bg-white/10 transition-all duration-300 whitespace-nowrap block',
                        hasDropdown
                          ? 'translate-y-0 opacity-100'
                          : '-translate-y-1 opacity-0'
                      )}
                      ...
                      link.subLinks?.map((sub) => {
                        ...
                        return (
                          <Link
                            ...
                            href={sub.href}
                            className={cn(
                              'pl-8 pr-4 py-1.5 text-white/70 text-sm rounded-lg hover:bg-white/10 hover:text-white transition-all duration-300 whitespace-nowrap flex items-center gap-2',
                              hasDropdown
                                ? 'translate-y-0 opacity-100'
                                : '-translate-y-1 opacity-0'
                            )}
