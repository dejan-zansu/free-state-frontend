...
interface ProductHeroLightProps {
      ...
      <div className="relative z-10 max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 pt-40 sm:pt-48 lg:pt-64 pb-8 lg:pb-0">
        <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-8 lg:gap-0">
          <div className="flex flex-col gap-4 sm:gap-5 max-w-lg">
            ...
            <h1 className="text-[#17302A] text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-medium leading-none tracking-tight whitespace-nowrap">
              {t('hero.title')}
            </h1>
          ...
          <div className="relative w-full lg:w-1/2 xl:w-[55%] aspect-[767/476] lg:absolute lg:right-0 lg:top-44">
            <div
              className="hidden lg:block absolute left-2/3 top-0 -translate-x-1/2 w-52 h-52 rounded-full z-0 pointer-events-none"
              style={{
                backgroundColor: 'rgba(183, 254, 26, 0.7)',
                filter: 'blur(120px)',
              }}
            />
            <Image
              src={imageSrc}
              alt={imageAlt ?? t('hero.title')}
              fill
...
export default ProductHeroLight
