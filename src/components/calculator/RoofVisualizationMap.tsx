...
const RoofVisualizationMap = forwardRef<
  RoofVisualizationMapRef,
...
>(({ className = '', showLegend = true, height = '300px' }, ref) => {
  ...
  const captureMapCanvas = (): string | null => {
    try {
      ...
      console.log(
        '[RoofVisualizationMap] Map element from ref:',
        mapElement ? 'exists' : 'null'
      )
      if (!mapElement) {
      ...
      console.log(
        '[RoofVisualizationMap] Element size:',
        rect.width,
        'x',
        rect.height
      )
      ...
      console.log(
        '[RoofVisualizationMap] Canvas size:',
        mapCanvas.width,
        'x',
        mapCanvas.height
      )
      ...
      canvases.forEach((canvas, index) => {
        console.log(
          `[RoofVisualizationMap] Canvas ${index}: ${canvas.width}x${canvas.height}`
        )
        if (canvas.width > 0 && canvas.height > 0) {
          ...
          try {
            ...
            console.error(
              `[RoofVisualizationMap] Error drawing canvas ${index}:`,
              drawError
            )
          }
        }
      ...
      console.log(
        '[RoofVisualizationMap] Generated data URL, length:',
        dataUrl.length
      )
      return dataUrl
    } catch (error) {
  ...
  return (
    <div className={`relative ${className}`}>
      ...
      {showLegend && (
        <div className="absolute bottom-2 right-2 bg-background/90 backdrop-blur-sm p-2 rounded-lg border text-sm">
