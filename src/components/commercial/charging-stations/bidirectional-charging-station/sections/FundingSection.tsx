            ...
            {cantons.map((canton, index) => (
              <div
                ...
                className={`grid grid-cols-2 border border-[#062E25]/40 backdrop-blur-[20px] ${
                  canton.highlighted ? 'bg-[#3D3858] text-white' : 'bg-white/10'
                } ${index === 0 ? 'rounded-t-2xl' : ''}`}
              >
                <span className={`px-[17px] py-3 text-lg font-normal tracking-[-0.02em] ${
                  canton.highlighted ? 'text-white/80' : 'text-[#062E25]/80'
                }`}>
                  {t(`funding.cantons.${canton.key}.name`)}
                </span>
                <span className={`px-5 py-3 text-lg font-semibold tracking-[-0.02em] ${
                  canton.highlighted ? 'text-white/80' : 'text-[#062E25]/80'
                }`}>
