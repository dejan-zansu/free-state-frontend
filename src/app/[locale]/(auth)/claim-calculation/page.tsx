...
export default function ClaimCalculationPage() {
  const invokedRef = useRef(false)
  const searchParams = useSearchParams()
  const token = searchParams.get('token')
  const router = useRouter()
  const locale = useLocale()
  const t = useTranslations('claimCalculation')
  ...
  }, [token, checkAuth, router, t, tErrors])
      ...
      <div className="w-full max-w-md">
        {status === 'loading' && (
          <div className="p-8 sm:p-10 text-center">
            <div className="mx-auto mb-6 flex h-16 w-16 items-center justify-center rounded-full bg-[#062E25]/5 ring-1 ring-[#062E25]/10">
              <Loader2 className="h-6 w-6 animate-spin text-[#062E25]" aria-hidden />
            </div>
            <h1 className="text-2xl font-bold tracking-tight text-[#062E25] sm:text-[1.75rem]">
              {t('verifying')}
            </h1>
            <p className="mt-3 text-[15px] text-muted-foreground">{t('verifyingHint')}</p>
          </div>
        )}
        ...
        {status === 'success' && (
          <div className="p-8 sm:p-10 text-center">
            ...
            <p className="mt-3 text-[15px] text-muted-foreground">{t('successMessage')}</p>
            <div className="mt-8">
