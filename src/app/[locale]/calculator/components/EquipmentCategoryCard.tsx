    ...
    <Card className='border-[#062E25]/10'>
      ...
      >
        ...
        <div className='flex items-center gap-3'>
          ...
          <ChevronDown className={cn('h-5 w-5 text-[#062E25]/30 transition-transform', expanded && 'rotate-180')} />
        </div>
      </button>
      ...
      {expanded && (
        <CardContent className='pt-0 pb-5 px-5'>
          {loading ? (
            <div className='py-8 text-center text-sm text-[#062E25]/40'>Loading...</div>
          ) : options.length === 0 ? (
            <div className='py-8 text-center text-sm text-[#062E25]/40'>No options available</div>
          ) : (
                  ...
                  >
                    <div className='flex gap-4 p-3'>
                      ...
                      <div className='flex-1 min-w-0 py-1'>
                        <div className='flex items-start justify-between gap-2'>
                          <div className='min-w-0'>
                            <div className='flex items-center gap-2'>
                              {isSelected && <Check className='h-4 w-4 text-[#062E25] shrink-0' />}
                              <span className='text-sm font-semibold text-[#062E25]'>{option.displayName}</span>
                            </div>
                            {option.manufacturer.name && (
                              <p className='text-sm text-[#062E25]/40 mt-0.5'>{option.manufacturer.name}</p>
                            )}
                          </div>
                        ...
                        <p className='text-sm text-[#062E25]/50 mt-1'>{option.specs}</p>
                      </div>
                    </div>
                  </button>
                )
