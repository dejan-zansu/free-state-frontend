...
export default function AdminMaintenanceInquiriesPage() {
                ...
                <TableBody>
                  {data.map(inquiry => (
                    <TableRow key={inquiry.id}>
                      ...
                      <TableCell className="text-sm text-[#062E25] max-w-48">
                        {inquiry.maintenanceServices.length > 0
                          ? inquiry.maintenanceServices.map(s => t(`serviceKeys.${s}`)).join(', ')
                          : '-'}
                      </TableCell>
                      <TableCell className="text-sm text-[#062E25]">
                        {inquiry.postalCode} {inquiry.location}
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        <Button variant="ghost" size="sm" asChild>
                          <Link href={`/${locale}/admin/maintenance-inquiries/${inquiry.id}`}>
                            {t('view')}
                          </Link>
                        </Button>
                      </TableCell>
                  ...
                  {data.length === 0 && (
                    <TableRow>
                      <TableCell
                        colSpan={6}
                        className="text-center py-8 text-[#062E25]"
                      >
                        {t('noInquiries')}
                      </TableCell>
                    </TableRow>
